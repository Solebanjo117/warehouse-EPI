...
public static partial class LabelDesignSerializer
{
    ...
    private static readonly HashSet<string> SystemBindings = new(StringComparer.Ordinal)
    {
        "product.sku", "product.description", "product.unit", "product.externalReference",
        "input.quantity", "input.manufacturingDate", "input.isRepack"
    };
    ...
    public static LabelDesignValidation Validate(LabelDesignDocumentV1? document, LabelSizePreset preset)
    {
        ...
        foreach (var field in document.Fields)
        {
            ...
            field.Options ??= [];
            if (!FieldKeyRegex().IsMatch(field.Key) || SystemBindings.Contains(field.Key)) errors.Add($"La clave de campo '{field.Key}' no es válida.");
        ...
        foreach (var element in document.Elements)
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(element.Binding) || !(SystemBindings.Contains(element.Binding) || keys.Contains(element.Binding)))
            ...
            if (element.Type == LabelElementType.Image && element.AssetId is null) errors.Add($"La imagen {element.Id} no tiene un recurso asociado.");
            if (element.Type != LabelElementType.Image && element.AssetId is not null) errors.Add($"El elemento {element.Id} no admite imágenes.");
