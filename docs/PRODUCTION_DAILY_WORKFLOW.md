...
## Cierre semanal en Balance

### Dos modalidades de captura

«Captura por escaneo» agrega cantidades a las ya registradas. «Tabla y balance» modifica el total del SKU, área y turno de una fecha: de 30 a 50 registra 20 adicionales. Ambas escriben mediante los mismos servicios y alimentan los mismos balances, reportes e historial.

En una semana abierta, ADMIN edita «Programado del día» directamente en un campo con el mismo resaltado al pasar el cursor o enfocar que T1/T2, sin desplegables. Si hay varios renglones, aparecen sus campos separados con unidad y referencias; solo se modifica la cantidad, sin repartir diferencias entre pedidos. Aperturas, extras y arrastres no se editan allí. Los cambios de programa y de producción se revisan juntos, con un máximo de 100 modificaciones. Cambiar el programa exige el NIP ADMIN de la sesión actual; reducir producción también exige motivo. La confirmación transaccional aplica primero reversos, después programación y finalmente las capturas nuevas o de reemplazo, con revisiones auditadas e idempotencia.

La vista previa calcula el balance diario y los tres bloques semanales en servidor sin guardar datos. Se identifica como «Vista previa sin guardar» y respeta filtros, unidades y corte dominical. Descartar restaura los valores guardados; la exportación siempre utiliza datos confirmados. Enter avanza a la siguiente celda editable y selecciona la cantidad. Cambiar de contexto con modificaciones pendientes requiere descartarlas explícitamente; los errores mantienen las cantidades introducidas y limpian el NIP.

La consulta del desglose y la edición de programación validan la sesión ADMIN en servidor. La confirmación vuelve a comprobar versiones de semana, líneas, capturas y órdenes. Ninguna modificación parcial se confirma si falla un reverso, un ajuste del plan o una captura. Los registros anteriores y los clientes que solo envían celdas T1/T2 conservan su contrato de guardado; no se añade migración.

Verificación de la integración: pruebas enfocadas de dominio, ruta y localización, más contratos JavaScript de captura y teclado. La suite PostgreSQL aislada incluye fallo inyectado de captura después del ajuste del plan para verificar rollback; en este entorno no pudo ejecutarse porque la cuenta de pruebas no tiene permiso `CREATE DATABASE`. La revisión visual y la prueba física siguen pendientes; no se inició ni reinició la aplicación.

### Reportes semanales

Debajo del balance de la fecha seleccionada se muestran siempre los encabezados plegables «Pendiente para próxima semana», «Cumplimiento semanal por producto y área» y «Resumen semanal de producción por SKU». Los tres se calculan al domingo de la semana seleccionada, independientemente de «Avance al día». Mientras la semana no esté cerrada son cifras provisionales; no anticipan capturas futuras. El pendiente toma la apertura del domingo, T1, saldo para T2, T2, saldo final firmado y por conciliar por SKU y área. Programar un arrastre en la semana siguiente es una acción independiente y no incrementa el saldo físico.

El cumplimiento agrupa los renglones repetidos por producto: completado por área entre la programación nueva semanal. Sin programación, el porcentaje se presenta como «Sin plan»; si el área no forma parte de la ruta, como «No aplica». Los porcentajes mayores a 100 % se conservan porque pueden contener trabajo de arrastres. Los totales se separan por unidad y se obtienen de cantidades, no promediando porcentajes. Pantalla y nuevas hojas `Pendiente proxima semana` y `Cumplimiento semanal` comparten la misma consulta y filtros; la paginación de 25 productos por sección solo afecta la pantalla.

La sección plegable «Comparación de turnos» reúne capturas activas de lunes a domingo por unidad y área, aunque Balance diario esté mostrando una fecha anterior. T1 y T2 se identifican por la configuración vigente; los reversos se excluyen. `% T1` y `% T2` son participaciones sobre T1 + T2; cuando ambos son cero se muestra «—». La fila Total suma trabajo de procesos y puede contar las mismas piezas en varias áreas. La hoja `Comparacion de turnos` exporta la misma población y filtros. En la tabla diaria, `Status %` se ubica después de Ready to Pack y calcula terminado hoy en esa área dividido entre programación nueva del día más pendiente inicial de Ready to Pack; sin base muestra 0 % y si el área no aplica, «No aplica». La vista previa de una corrección actualiza este porcentaje; el XLSX solo contiene valores guardados.

Las tres secciones del cierre están plegadas inicialmente para que la vista de Balance no acumule tres tablas largas. «Resumen semanal de producción por SKU» muestra programación nueva y piezas completadas por área. «Cumplimiento semanal por producto y área» agrega los porcentajes al completado. El resumen omite las diferencias «restantes» del Excel porque pueden confundirse con el pendiente físico por etapa, que se consulta en «Pendiente para próxima semana». Cada sección pagina 25 SKU de manera independiente; sus totales no cambian al paginar. La exportación mantiene la hoja `Resumen produccion semanal` con plan y completado, sin retirar las hojas existentes.

### Alta de programación desde cero en Balance

ADMIN puede escribir en «Programado del día» cuando vale 0 y no existe programación nueva para ese SKU y fecha. La cantidad positiva se revisa como «Nueva programación» y se confirma con el mismo NIP y transacción del grupo. Se crea un renglón de inventario sin referencias ni notas mediante el servicio de Programa semanal; no convierte pendientes ni arrastres en programa. Volver a 0 o descartar antes de confirmar no crea registros. Un renglón existente sigue requiriendo cantidad positiva.

La vista previa recalcula el balance y los bloques semanales sin escribir. El servidor valida producto activo, unidad, fecha, sesión ADMIN, versión de semana y ausencia de otra línea nueva para el día. Una alta concurrente bloquea el grupo; no combina pedidos ni duplica líneas. Los reintentos conservan el identificador de operación. Las pruebas incluyen lunes/domingo, altas solas y mixtas, exportación, permisos, cero, unidad y doble envío; la prueba PostgreSQL aislada incluye fallo inducido después del alta para verificar rollback de líneas, órdenes y revisiones.

Validación de esta alta: 32 pruebas .NET y 7 JavaScript aprobadas. La ejecución PostgreSQL aislada se detuvo al crear la base temporal por permiso 42501; la prueba de rollback queda pendiente de ese entorno. Revisión visual en navegador y prueba física en tablet pendientes. No se desplegó ni se reiniciaron servicios.
