...
public sealed class ProductionServiceTests
{
    ...
    private sealed class Fixture:IAsyncDisposable
    {
        ...
        public static async Task<Fixture>CreateAsync(int stageCount){var db=new WarehouseDbContext(new DbContextOptionsBuilder<WarehouseDbContext>().UseInMemoryDatabase(Guid.NewGuid().ToString()).Options);await db.Database.EnsureCreatedAsync();var pins=new UserPinService(db,new PinProtector(Key));var admin=new User{FullName="Admin producción",RoleId=1,PinLookup="",PinHash=""};var op=new User{FullName="Operador producción",RoleId=2,PinLookup="",PinHash=""};await pins.AssignAsync(admin,"4826");await pins.AssignAsync(op,"5937");db.Users.AddRange(admin,op);var service=new ProductionService(db,pins,new InventoryMovementService(db,pins,TimeProvider.System),TimeProvider.System);var f=new Fixture(db,service);f.Product=new Product{Sku="FG-PROD",BaseUnitId=1};f.Shift=new ProductionShift{Code="T1",Name="Turno 1"};f.Location=new Location{Code="PT-1",Kind=LocationKind.Area};db.AddRange(f.Product,f.Shift,f.Location);var stages=Enumerable.Range(1,stageCount).Select(i=>new ProductionStage{Code=$"E{i}",Name=$"Etapa {i}"}).ToArray();db.AddRange(stages);await db.SaveChangesAsync();var route=await service.CreateRouteAsync(new(Guid.NewGuid(),f.Product.Id,"Ruta prueba",stages.Select(x=>x.Id).ToArray(),f.AdminPin));Assert.Equal(ProductionCommandStatus.Success,route.Status);return f;}
        public async Task<Guid>CreateAndReleaseAsync(decimal quantity){var created=await Service.CreateOrderAsync(new(Guid.NewGuid(),Product.Id,quantity,null,null,null,AdminPin));Assert.Equal(ProductionCommandStatus.Success,created.Status);var released=await Service.ReleaseAsync(new(Guid.NewGuid(),created.WorkOrderId!.Value,null,AdminPin));Assert.True(released.Status==ProductionCommandStatus.Success,string.Join(" | ",released.ValidationErrors));return created.WorkOrderId.Value;}
