...
public sealed class ProductionService(WarehouseDbContext db, UserPinService pins, InventoryMovementService movements, TimeProvider timeProvider)
{
    ...
    public async Task<ProductionCommandResult> ReceiveWarehouseAsync(ReceiveProductionToWarehouseCommand command,CancellationToken token=default)
    {
        ...
        try{
            ...
            var p=BuildProgress(order).Single(x=>x.Id==final.Id); if(command.Quantity>p.AvailableToDeliver)return await AbortAsync(tx,Invalid("La cantidad excede el producto terminado disponible."),token);
            var movement=await movements.ConfirmAuthorizedAsync(new InventoryMovementCommand(command.OperationId,InventoryMovementType.Entry,command.Pin,[new(order.ProductId,command.Quantity,DestinationLocationId:command.DestinationLocationId)],order.Number,"Recepción de producción",command.ApprovedSharedAssignments,InventoryMovementPurpose.ProductionReceipt),user,token);
            if(movement.Status!=InventoryMovementStatus.Success)return await AbortAsync(tx,MapMovement(movement,order.Id),token);
            order.Status=ProductionWorkOrderStatus.InProgress; var receiptEvent=Event(command.OperationId,fp,order,user,ProductionEventType.WarehouseReceived,final.Id,quantity:command.Quantity,movementId:movement.MovementId); order.Events.Add(receiptEvent); db.Entry(receiptEvent).State=EntityState.Added; order.Version++; await db.SaveChangesAsync(token); if(tx is not null)await tx.CommitAsync(token); return new(ProductionCommandStatus.Success,order.Id,movement.MovementId);
    ...
    private async Task<ProductionCommandResult> HandoffAsync(ProductionHandoffCommand command,ProductionEventType type,CancellationToken token,bool admin=false,bool lost=false)
    {
        ...
        return await MutateAsync(command.OperationId,command.WorkOrderId,fp,async order=>{
            ...
            var p=BuildProgress(order).Single(x=>x.Id==source.Id);var available=type==ProductionEventType.Delivered?p.AvailableToDeliver:p.PendingReceipt;if(command.Quantity>available)return Invalid(type==ProductionEventType.Delivered?"La cantidad excede lo disponible para entregar.":"La cantidad excede lo pendiente de recepción.");
            order.Status=ProductionWorkOrderStatus.InProgress;order.Events.Add(Event(command.OperationId,fp,order,user,type,source.Id,target.Id,quantity:command.Quantity,reason:command.Reason));await Task.CompletedTask;return Success(order.Id);
        },token);
    }
