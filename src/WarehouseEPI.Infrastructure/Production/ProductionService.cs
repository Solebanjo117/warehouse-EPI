...
public sealed class ProductionService(WarehouseDbContext db, UserPinService pins, InventoryMovementService movements, TimeProvider timeProvider)
{
    ...
    public async Task<ProductionCommandResult> ReceiveWarehouseAsync(ReceiveProductionToWarehouseCommand command, CancellationToken token = default)
    {
        ...
        catch (DbUpdateConcurrencyException) { return await AbortAsync(tx, new(ProductionCommandStatus.ConcurrencyConflict), token); }
        catch { if (tx is not null) await tx.RollbackAsync(token); throw; }
    }
    ...
    private async Task<User?> OperatorAsync(string pin, CancellationToken t) { var u = await pins.AuthenticateAsync(pin, t); return u?.Role.Code is "ADMIN" or "OPERATOR" ? u : null; }
    private async Task<User?> AdminAsync(string pin, CancellationToken t) { var u = await pins.AuthenticateAsync(pin, t); return u?.Role.Code == "ADMIN" ? u : null; }
    ...
    internal static IReadOnlyList<ProductionStageProgress> BuildProgress(ProductionWorkOrder order)
    {
        var ordered = order.Stages.OrderBy(x => x.Sequence).ToArray(); var list = new List<ProductionStageProgress>(); var reversed = order.Events.Where(x => x.Type == ProductionEventType.ResultReversed && x.RelatedEventId.HasValue).Select(x => x.RelatedEventId!.Value).ToHashSet(); var effective = order.Events.Where(x => !reversed.Contains(x.Id) && x.Type != ProductionEventType.ResultReversed).ToArray(); foreach (var s in ordered)
        {
            ...
            list.Add(new(s.Id, s.Sequence, s.Code, s.Name, input - processed, processed, good, rework, scrap, good - delivered + returned - warehouse, delivered, received, pending, inputSince, goodSince));
        }
        return list;
    }
