(() => {
  ...
  const itemSnapshot = () => elements.map((element) => ({
    ...
    isDashed: element.dataset.dashed === "true", zIndex: number(element, "z"), isLocked: element.dataset.elementLocked === "true", isVisible: element.dataset.visible !== "false", persisted: element.dataset.persisted === "true"
  }));
  ...
  const snapshot = () => ({ items: itemSnapshot(), layers: layerSnapshot() });
  ...
  const updateFields = () => {
    if (field) field.value = JSON.stringify(itemSnapshot().filter((item) => !item.architecture).map((item) => ({ Id: item.id, X: item.x, Y: item.y, Width: item.width, Height: item.height, Rotation: item.rotation, ZIndex: item.zIndex, IsVisible: item.isVisible })));
    if (architectureField) architectureField.value = JSON.stringify(itemSnapshot().filter((item) => item.architecture).map((item) => ({
      ...
      IsDashed: item.isDashed, ZIndex: item.zIndex, IsLocked: item.isLocked
    })));
  ...
  const createArchitectureElement = (item) => {
    ...
    Object.assign(element.dataset, { layerCode: item.layerCode, kind: item.kind, label: item.label || "", x: item.x, y: item.y, width: item.width, height: item.height, rotation: item.rotation || 0, radius: item.radius || 0, points: pointsText(item.points || []), strokeToken: item.strokeToken, fillToken: item.fillToken, strokeWidth: item.strokeWidth, dashed: String(item.isDashed), z: item.zIndex, elementLocked: "false", persisted: String(item.persisted === true) });
  ...
  const renderAll = () => { elements.forEach(renderElement); updateSelection(); updateFields(); };
  const restore = (state) => {
    const architectureState = state.items.filter((item) => item.architecture); const architectureIds = new Set(architectureState.map((item) => item.id)); architectureElements.filter((item) => !architectureIds.has(elementId(item))).forEach(removeArchitectureElement);
    state.items.forEach((item) => { let element = elements.find((value) => elementId(value) === item.id && isArchitecture(value) === item.architecture); if (!element && item.architecture) element = createArchitectureElement({ ...item, points: parsePoints(item.points), radius: item.radius }); if (!element) return; Object.assign(element.dataset, { layerCode: item.layerCode, kind: item.kind, label: item.label, x: item.x, y: item.y, width: item.width, height: item.height, rotation: item.rotation, radius: item.radius, points: item.points, strokeToken: item.strokeToken, fillToken: item.fillToken, strokeWidth: item.strokeWidth, dashed: String(item.isDashed), z: item.zIndex, elementLocked: String(item.isLocked), visible: String(item.isVisible), persisted: String(item.persisted) }); });
  ...
  const newElement = (definition, origin) => createArchitectureElement({ id: uuid(), ...definition, label: definition.kind === "Text" ? "Texto" : "", x: origin.x, y: origin.y, width: definition.kind === "Text" ? 160 : 1, height: definition.kind === "Text" ? 24 : 1, radius: 0, points: [], rotation: 0, zIndex: nextZ(), persisted: false });
  ...
  const cancelActiveTool = () => { if (interaction?.kind?.startsWith("draw")) { restore(interaction.before); interaction = null; editor.querySelector("[data-editor-finish]").hidden = true; editor.querySelector("[data-editor-cancel]").hidden = true; status("Dibujo cancelado."); clearGuides(); return true; } return false; };
  const setTool = (value) => {
    if (interaction?.kind?.startsWith("draw")) cancelActiveTool(); const definition = toolDefinitions[value]; if (definition && layerIsLocked(definition.layerCode)) { status(`Desbloquea la capa ${definition.layerCode} antes de dibujar.`); return; }
  ...
  function bindElement(element) {
    element.addEventListener("pointerdown", (event) => { if (tool !== "select" || elementIsLocked(element) || !layerIsVisible(layerCode(element))) return; event.preventDefault(); event.stopPropagation(); const additive = event.ctrlKey || event.metaKey || multiMode; if (additive) toggleSelection(element); else if (!selected.has(element)) setSelection([element], element); else { activeElement = element; updateSelection(); } if (selected.has(element)) beginTransform(event, !isArchitecture(element) && event.target.classList.contains("editor-resize-handle") ? "resize" : "move"); });
  }
  ...
  svg.addEventListener("pointerdown", (event) => {
    ...
    if (event.target.closest("[data-editor-element], [data-architecture-element]") || event.target.closest("[data-editor-selection]")) return;
    ...
    if (tool !== "select") return; if (!event.ctrlKey && !event.metaKey && !multiMode) clearSelection(); const start = point(event); interaction = { pointer: event.pointerId, kind: "marquee", start, changed: false }; svg.setPointerCapture(event.pointerId);
  });
  ...
  const endPointer = (event) => {
    ...
    if (interaction.kind === "marquee") { const marquee = selectionLayer.querySelector(".editor-marquee"); if (marquee) { const left = Number(marquee.getAttribute("x")); const top = Number(marquee.getAttribute("y")); const right = left + Number(marquee.getAttribute("width")); const bottom = top + Number(marquee.getAttribute("height")); const matches = operationalElements.filter((element) => !elementIsLocked(element) && number(element, "x") < right && number(element, "x") + number(element, "width") > left && number(element, "y") < bottom && number(element, "y") + number(element, "height") > top); setSelection(matches); marquee.remove(); } interaction = null; return; }
  ...
  const organize = (minimum, apply) => { const items = operationalSelection(); if (items.length < minimum || items.length !== selected.size) return; pushUndo(); apply(items); renderAll(); };
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if (modifier && event.key.toLowerCase() === "z") { event.preventDefault(); if (event.shiftKey) redoAction(); else undoAction(); return; } if (modifier && event.key.toLowerCase() === "y") { event.preventDefault(); redoAction(); return; } if (event.key === "Escape") { if (!cancelActiveTool()) { clearSelection(); setTool("select"); } return; } if (event.key === "Enter" && interaction?.kind === "drawPolyline") { event.preventDefault(); finishPolyline(); return; }
  ...
  restoreVisibility(); applyWorkspacePreferences(); layerButtons.forEach((button) => setLayerLocked(button.dataset.editorLayerLock, button.getAttribute("aria-pressed") === "true")); applyViewBox(); renderAll();
})();
