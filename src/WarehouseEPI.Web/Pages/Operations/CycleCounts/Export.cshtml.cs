...
public sealed class ExportModel(CycleCountService cycleCountService, ReportExportService exportService, IStringLocalizer<OperationsTexts> texts) : PageModel
{
    public async Task<IActionResult> OnGetAsync(Guid id, string format, CancellationToken cancellationToken)
    {
        if (format is not ("csv" or "xlsx")) return BadRequest(texts["El formato debe ser csv o xlsx."]);
        var rows = await cycleCountService.GetExportRowsAsync(id, 10000, cancellationToken);
        if (rows.Count > 10000) return BadRequest(texts["La campaña supera el límite de 10,000 líneas; no se generó un archivo parcial."]);
