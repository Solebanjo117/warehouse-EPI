...
public sealed class ProductionBlockBPostgreSqlTests(PostgreSqlInventoryFixture fixture)
{
    ...
    public async Task Queues_page_whole_orders_in_same_priority_order_and_detect_content_changes()
    {
        ...
        for(var i=0;i<27;i++) {
            var order=new ProductionWorkOrder {Number=$"B-{suffix}-{i:00}",CreateOperationId=Guid.NewGuid(),CreateFingerprint=$"b-{i}",ProductId=seed.ProductId,UnitId=1,CreatedByUserId=user.Id,
                TargetQuantity=10,AuthorizedQuantity=10,OriginalTargetQuantity=10,Status=ProductionWorkOrderStatus.Released,
                SupplyPriority=i%3==0?ProductionSupplyPriority.Urgent:ProductionSupplyPriority.Normal,
                DueDate=i%2==0?new DateOnly(2026,9,20):null,CreatedAt=new DateTimeOffset(2026,9,1,0,0,0,TimeSpan.Zero).AddMinutes(i)};
