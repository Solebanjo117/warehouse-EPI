...
public class LocationDetailsPageModel(
    WarehouseDbContext dbContext,
    ProductLocationAssignmentService assignmentService) : PageModel
{
    protected WarehouseDbContext DbContext { get; } = dbContext;
    protected ProductLocationAssignmentService AssignmentService { get; } = assignmentService;
    ...
    {
        ProductSearch = IsAdministrativeView ? productSearch?.Trim() : null;
        if (!await LoadAsync(id, cancellationToken)) return NotFound();
        return Page();
    }

    protected async Task<bool> LoadAsync(Guid id, CancellationToken cancellationToken)
    {
    ...
    protected void SetResultMessage(ProductLocationAssignmentResult result)
    {
