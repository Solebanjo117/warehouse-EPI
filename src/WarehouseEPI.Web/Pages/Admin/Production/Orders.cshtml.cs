...
public sealed class OrdersModel(ProductionQueryService query,ProductionService service):PageModel
{
 [BindProperty(SupportsGet=true)]public string? Search{get;set;} [BindProperty]public InputModel Input{get;set;}=new(); public IReadOnlyList<ProductionOrderRow> Items{get;private set;}=[];public List<Product> Products{get;private set;}=[];
 public async Task OnGetAsync(CancellationToken t)=>await LoadAsync(t);
 public async Task<IActionResult> OnPostAsync(CancellationToken t){if(ModelState.IsValid){var r=await service.CreateOrderAsync(new(Input.OperationId,Input.ProductId,Input.TargetQuantity,Input.ExternalReference,Input.DueDate,Input.Notes,Input.Pin),t);Input.Pin="";if(r.Status==ProductionCommandStatus.Success)return RedirectToPage("Details",new{id=r.WorkOrderId});ModelState.AddModelError(string.Empty,r.ValidationErrors.FirstOrDefault()??"NIP ADMIN inválido o datos no válidos.");}Input.Pin="";await LoadAsync(t);return Page();}
