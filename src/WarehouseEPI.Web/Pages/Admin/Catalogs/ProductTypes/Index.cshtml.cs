...
public sealed class IndexModel(WarehouseDbContext dbContext, IStringLocalizer<CatalogTexts> text) : PageModel
{
    ...
    public async Task<IActionResult> OnPostSaveAsync(CancellationToken token)
    {
        ...
        if (await dbContext.ProductTypes.AnyAsync(x => x.Code == Input.Code && x.Id != Input.Id, token)) { ModelState.AddModelError("Input.Code", text["Ya existe un tipo con ese código."].Value); await LoadAsync(token); return Page(); }
    ...
    public async Task<IActionResult> OnPostToggleAsync(short id, CancellationToken token) { var x = await dbContext.ProductTypes.SingleOrDefaultAsync(y => y.Id == id, token); if (x is null) return NotFound(); if (x.IsActive) { var count = await dbContext.Products.CountAsync(p => p.IsActive && p.ProductTypeId == id, token); if (count > 0) { TempData["Error"] = text["No se puede desactivar: {0} producto(s) activo(s) usan este tipo.", count].Value; return RedirectToPage(); } } x.IsActive = !x.IsActive; await dbContext.SaveChangesAsync(token); return RedirectToPage(); }
