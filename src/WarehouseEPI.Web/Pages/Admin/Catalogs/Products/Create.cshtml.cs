...
public sealed class CreateModel(WarehouseDbContext dbContext, ProductionWipDefaultService wipDefaults, IStringLocalizer<CatalogTexts> text) : PageModel, IProductFormPage
{
    ...
    public async Task<IActionResult> OnPostAsync(string? nextStep, CancellationToken token)
    {
        ...
        if (attempted.Length > 0 && (string.IsNullOrWhiteSpace(Wip.Reason) || string.IsNullOrWhiteSpace(Wip.Pin)))
            ModelState.AddModelError("Wip.Reason", text["Indica motivo y NIP ADMIN para guardar destinos WIP."].Value);
        ...
        TempData["Success"] = text["Producto creado."].Value;
    ...
    private string WipError(WipDefaultResult result) => result.Status switch
    {
