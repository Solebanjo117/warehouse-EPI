...
public sealed class IndexModel(WarehouseDbContext dbContext, IStringLocalizer<CatalogTexts> text) : PageModel
{
    ...
    public async Task<IActionResult> OnPostSaveAsync(CancellationToken token) { Input.Code = CatalogNormalization.NormalizeCode(Input.Code); Input.Name = Input.Name.Trim(); if (!ModelState.IsValid) { await LoadAsync(token); return Page(); } if (await dbContext.ProductClasses.AnyAsync(x => x.Code == Input.Code && x.Id != Input.Id, token)) { ModelState.AddModelError("Input.Code", text["Ya existe una clase con ese código."].Value); await LoadAsync(token); return Page(); } if (Input.Id == 0) dbContext.ProductClasses.Add(new ProductClass { Code = Input.Code, Name = Input.Name }); else { var x = await dbContext.ProductClasses.SingleOrDefaultAsync(y => y.Id == Input.Id, token); if (x is null) return NotFound(); x.Code = Input.Code; x.Name = Input.Name; } await dbContext.SaveChangesAsync(token); return RedirectToPage(); }
    public async Task<IActionResult> OnPostToggleAsync(short id, CancellationToken token) { var x = await dbContext.ProductClasses.SingleOrDefaultAsync(y => y.Id == id, token); if (x is null) return NotFound(); if (x.IsActive) { var count = await dbContext.Products.CountAsync(p => p.IsActive && p.ProductClassId == id, token); if (count > 0) { TempData["Error"] = text["No se puede desactivar: {0} producto(s) activo(s) usan esta clase.", count].Value; return RedirectToPage(); } } x.IsActive = !x.IsActive; await dbContext.SaveChangesAsync(token); return RedirectToPage(); }
