...
public sealed class ProductionBlockAPostTests
{
    ...
    public async Task Real_form_preserves_bad_decimal_clears_pin_and_activates_explicit_rework()
    {
        ...
        var fields = new Dictionary<string,string> {
            ["__RequestVerificationToken"]=Token(html), ["BatchResult.OperationId"]=Guid.NewGuid().ToString(),
            ["BatchResult.BatchId"]=batch.Id!.Value.ToString(), ["BatchResult.StageId"]=order.Stages.Single().Id.ToString(),
            ["BatchResult.ShiftId"]=shift.Id.ToString(), ["BatchResult.ExpectedVersion"]=order.Version.ToString(),
            ["BatchResult.InputQuantity"]="0,25", ["BatchResult.GoodQuantity"]="0", ["BatchResult.ReworkQuantity"]="5",
            ["BatchResult.ScrapQuantity"]="0", ["BatchResult.Pin"]="6724", ["BatchResult.IsRework"]="false",
